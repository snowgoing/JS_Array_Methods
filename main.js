...
// Question 1: Find average price of all items.
/*
var sum = 0;
var x = 0;
function avg(item) {
...
var a, sum = 0;
var x = items.forEach(function(value) {
  sum += value.price;
});

x = "The average price is $" + (sum / items.length).toFixed(2);

document.querySelector("#question1").innerHTML = "<p>"+x+"</p>";
...
/*
// Question 2: Array of items prices between $14 and $18.

var sum = [];
function between14and18(value) {
  for(var i = 0; i < items.length; i++) {
    if(value[i].currency_code && value[i].price > 14 && value[i].price < 18) {
      sum.push(value[i].title)
    }
  }
      console.log(sum);
      return sum;
...
between14and18(items);
*/
var prices = [];
items.filter(function(value) {
  if(value.price > 14 && value.price < 18) {
    prices.push(value.title);
    document.querySelector("#question2").innerHTML = "<p>"+prices[0]+"</p>"+"<p>"+prices[1]+"</p>"+"<p>"+prices[2]+"</p>";
  }
});


/*
//Question 3: Display the name and price of the item listed in GBP.

var x = "";
function gbp(value) {
  for(var i = 0; i < items.length; i++) {
    if(value[i].currency_code === "GBP") {
      x = value[i].title + " costs £" + value[i].price; 
    }
  }
    console.log(x);
    return x;
...
items.forEach(function(value){
  if(value.currency_code === "GBP") {
    document.querySelector("#question3").innerHTML = "<p>"+value.title+' costs £'+value.price+"</p>";
  }
})


/*
// Question 4: Display all items made of wood. 

var wood = [];
x = ""
function woodMade(value) {
  for(var i = 0; i < items.length; i++) {
    for(var j = 0; j < items[i].materials.length; j++) {
      if(value[i].materials[j] === "wood") {
      wood.push("<br>"+value[i].title + " is made of wood.");
      }
    }
  }
    console.log(wood);
...
*/
var myArr = [];
var woodMade = items.map(function(val) {
  var num = val.materials.length;
  for(var i = 0; i < num; i += 1) {
    if(val.materials[i] === "wood") {
      myArr.push(val.title);
    }
  }
    return myArr;
}).forEach(function() {
  var i = 0;
  document.querySelector("#question4").innerHTML = "<p>"+myArr[0]+" is made of wood"+"</p>"+
                                                    "<p>"+myArr[1]+" is made of wood"+"</p>"+
                                                    "<p>"+myArr[2]+" is made of wood"+"</p>"+
                                                    "<p>"+myArr[3]+" is made of wood"+"</p>"+
                                                    "<p>"+myArr[4]+" is made of wood"+"</p>"
});


/*
// Question 5: Items made with 8 or more materials. Display title, quantity and materials.

var sum = [];
function materialsX8(value) {
  for (var i = 0; i < items.length; i++) {
    if (value[i].materials.length >= 8) {
        sum.push(value[i].title + " has " + value[i].materials.length + " materials:", value[i].materials)
    }
  }
  console.log(sum);
  return sum;
...
document.querySelector("#question2").innerHTML = "<p>"+materialsX8(items)+"</p>";
materialsX8(items)
*/
...
items.filter(function(value) {
  if(value.materials.length >= 8) {
    var x = value.materials.length;
    eightOrMore.push(value.title+" has "+x+" materials:"+"<br>"+value.materials+"<br>");
    document.querySelector("#question5").innerHTML = "<p>"+eightOrMore+"</p>";
  }
});
...
sum = 0;
function sellerMade(value) {
  for (var i = 0; i < items.length; i++) {
    if(items[i].who_made === "i_did") {
      sum += 1;
    }
  }  
    console.log(sum + " were made by their sellers");
    return sum + " were made by their sellers";
}

document.querySelector("#question1").innerHTML = "<p>"+sellerMade(items)+"</p>";
...
document.querySelector("#question6").innerHTML = "<p>"+sum+' were made from their sellers'+"</p>"
